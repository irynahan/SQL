...
use sakila;
insert into customer (first_name, last_name, address_id, email, store_id, create_date)
select first_name, last_name, address_id, email, 2, now()
from employee
...
where a.last_name = 'lollobrigida') and actor.last_name != "lollobrigida"
...
use sakila;
update employee set first_name = "Oleg", email = "qwerty@gmail.com"
where employee_id = 3;
...
where cu.customer_id in (select c.customer_id
from customer c
join payment p using (customer_id)
...
update positions p
join employee e using(position_id)
set p.name = 'Senior Java Developer'
where concat(first_name, " ", last_name) in ('Paup Trout','Ura Urch')
